...
const SplashScreen = () => {
  ...
  useEffect(() => {
    const animateCode = () => {
      const deleteCode = () => {
        if (highlightRef.current && linesGroupRef.current && codeBubbleRef.current) {
          highlightRef.current.style.display = "none";
          linesGroupRef.current.style.display = "none";
          codeBubbleRef.current.classList.remove("animating");
      ...
      const startTyping = () => {
        if (linesGroupRef.current && codeBubbleRef.current) {
          linesGroupRef.current.style.display = "block";
          const lines = linesGroupRef.current.querySelectorAll("path");

          lines[lines.length - 1].addEventListener("animationend", selectAll);
          codeBubbleRef.current.classList.add("animating");
        }
      };
...
<div className={isLoading ? 'splash-screen' : 'hidden'}>
    <svg class="code-animation" viewBox="0 0 1172 1489" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path
        fill-rule="evenodd"
        clip-rule="evenodd"
  ...
  ref={highlightRef}
        ...
        fill-rule="evenodd"
        clip-rule="evenodd"
        d="M36.8906 165.151H874.59V1447.87H36.8906V165.151Z"
        fill="#54E3EB"
        />
</svg>
</div>
